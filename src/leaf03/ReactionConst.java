...
public class ReactionConst extends Reaction {

	public double rate;
	
	public ReactionConst(Context<PhysicalAgent> context, Node reactionNode) {
			...
			for (int x = 0; x < width; x++) {
				...
				for (PhysicalAgent b : agentList) {
					if (b.hasReaction(name)) {
						...
						double newAgentMass = agentMass + rate * dt * biomassRelativeYield;
